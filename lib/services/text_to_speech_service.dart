...
class TextToSpeechService {
  ...
  Future<void> _initTts() async {
    ...
    try {
      ...
      TtsGoogle.init(apiKey: _apiKey, withLogs: true);
  ...
  Future<void> speak(String text) async {
    ...
    try {
      ...
      dynamic voice;
      final oliverVoice =
          voices.where((v) => v.name.toLowerCase().contains('oliver')).toList();

      if (oliverVoice.isNotEmpty) {
        voice = oliverVoice.first;
        logInfo(
          'Utilisation de la voix oliver (fr-CA) pour la lecture directe',
        );
      } else {
        final frenchVoices =
            voices.where((v) => v.locale.code.startsWith("fr-")).toList();
      ...
      final tempFile = File(
        '${tempDir.path}/temp_speak_${DateTime.now().millisecondsSinceEpoch}.mp3',
      );
  ...
  Future<AudioFile?> convertTextToMP3WithCustomName(
    ...
    try {
      ...
      if (oliverVoice.isNotEmpty) {
        voice = oliverVoice.first;
        logInfo(
          'Utilisation de la voix oliver (fr-CA) - voix testée et fonctionnelle',
        );
      } else {
        final frenchVoices =
            voices.where((v) => v.locale.code.startsWith("fr-")).toList();
      ...
      logInfo('  - Format: MP3');
      logInfo(
        '  - Texte: "${text.substring(0, text.length.clamp(0, 30))}${text.length > 30 ? '...' : ''}"',
      );
